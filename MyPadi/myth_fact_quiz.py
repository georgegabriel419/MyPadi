...
def main():
    ...
    with open("myth_questions.json", "r", encoding="utf-8") as f:
        quiz_data = json.load(f)

    language_map = {
        "English": ("English", quiz_data.get("English", [])),
        "Yoruba": ("Yoruba", quiz_data.get("Yoruba", [])),
        "Igbo": ("Igbo", quiz_data.get("Igbo", [])),
        "Hausa": ("Hausa", quiz_data.get("Hausa", [])),
        "Pidgin": ("Pidgin English", quiz_data.get("Pidgin", []))
    }
    ...
    if not st.session_state.started:
        ...
        if st.button("✅ Start Quiz"):
            if lang_choice == "Select Language" or age_choice == "Select Age Group":
                ...
                filtered = [q for q in data if q["age_group"] == age_choice]
                random.shuffle(filtered)
                st.session_state.questions = filtered[:10]
...
# 🟣 This ensures the quiz runs if you call it from another file
